...
//Ability grade object literal.

//This array houses the grade options.
//const gradePicks = [blank2, omega, alpha, spr, a, b, c, d];
...
function gradeDropDown(ability) {
  ...
  //Get grade selection.
  let gradeChoice = document.getElementById('selectGrade');
...
/*function purchaseAbility() {
  ...
  let purchase = gradePool * abilityCost;
  if (budget < 1) {
    document.write("You're out of points");
  } else if (budget < purchase) {
    document.write("You don't have enough points");
  } else if (budget >= purchase) {
    budget -= purchase;
    console.log("You now have " + budget + " points remaing.");
  }
}*/
